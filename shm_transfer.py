"""Layer-wise IPC transfer of GPU-resident paged KV cache via /dev/shm.
...
Sender:
  ...
  2. For each layer, gathers the target requests' KV on the GPU, creates
     a sized file under /dev/shm, mmap's it, pins the payload region
     (cudaHostRegister under the hood via torch pin_memory), and performs
     a single synchronous D2H copy into the pinned mmap.  No separate
     pinned staging buffer or host-to-host copy.
...
Receiver:
  ...
  2. Returns zero-copy tensor views backed by the mmap'd shm file
...
def _recv_shm_path(sock: socket.socket) -> str:
    (n,) = struct.unpack(">I", _recv_exact(sock, PATH_LEN_SIZE))
    ...
    with socket.socket(socket.AF_UNIX, socket.SOCK_STREAM) as sock:
        ...
        for layer_idx in range(pool.num_layers):
            ...
            shm_path = os.path.join(
                shm_dir, f"kv_layer_{os.getpid()}_{layer_idx}_{token}"
            )
...
def main():
    parser = argparse.ArgumentParser(
        description="Layer-wise paged KV cache transfer via /dev/shm files over Unix domain sockets.",
    )
    ...
    parser.add_argument("--socket-path", default="/tmp/shm.sock")
    parser.add_argument(
        "--shm-dir",
        default="/dev/shm",
        help="Directory for per-layer shm files (sender only).",
    )
    parser.add_argument(
        "--model",
