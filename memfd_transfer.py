...
Sender:
  ...
  2. For each layer, gathers the target requests' KV on the GPU, creates
     an mmap'd memfd (size rounded up to a host page), registers the
     mapping with cudaHostRegister so the buffer is pinned for CUDA, then
     performs a synchronous D2H copy into the payload.  No separate
     staging buffer or host-to-host copy.
